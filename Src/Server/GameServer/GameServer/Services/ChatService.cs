...
namespace GameServer.Services
{
    class ChatService : Singleton<ChatService>
    {
        public ChatService()
        {
            MessageDistributer<NetConnection<NetSession>>.Instance.Subscribe<ChatRequest>(this.OnChatRequest);
        }
        ...
        private void OnChatRequest(NetConnection<NetSession> sender, ChatRequest request)
        {
            Character character = sender.Session.Character;
            Log.InfoFormat("OnChatRequest: character:{0} Channel:{1} Message:{2}", character.Id, request.Message.Channel, request.Message.Message);
