...
namespace GameServer.Services
{
    class QuestService : Singleton<QuestService>
    {
        ...
        private void OnQuestAccept(NetConnection<NetSession> sender, QuestAcceptRequest request)
        {
            ...
            Log.InfoFormat("QuestAcceptRequest: :character:{0}:Quest{1}",character.Id, request.QuestId);
        ...
        {
            Character character = sender.Session.Character;
            Log.InfoFormat("QuestSubmitRequest: :character:{0}:Quest{1}", character.Id, request.QuestId);
