...
namespace Services
{
    class StatusService : Singleton<StatusService>, IDisposable
    {
        ...
        public void RegisterStatuesNotify(StatusType function, StatusNotifyHandler action)
        {
        ...
        private void Notify(NStatus status)
        {
            ...
            StatusNotifyHandler hander;
            if (eventMap.TryGetValue(status.Type, out hander))
            {
                hander(status);
            }
        }
